...
Create one server Custom trigger:

**Name:** `CE - generate_lead - backend`
...
Use this trigger for the server conversion tags below.
...
Trigger: `CE - generate_lead - backend`.
...
If you choose backend-only GA4 for `generate_lead`, create a server GA4 tag on `CE - generate_lead - backend` and map only analytics-safe event fields. Do not pass the `user_data` object, email, phone, IP, or other PII to GA4.
...
3. Submit the Measurement Stack demo form.
4. Confirm an incoming Data Client request at `/data` with `event_name = generate_lead`.
5. Confirm the incoming `event_id` matches the browser `dataLayer` `generate_lead.event_id`.
6. Confirm Meta CAPI and LinkedIn CAPI tags fire once.
7. Check Stape Logs for incoming request + associated outgoing requests.
8. Check Meta Test Events / Events Manager and LinkedIn Campaign Manager diagnostics.
9. After validation, publish the server GTM container and remove any temporary test codes.
